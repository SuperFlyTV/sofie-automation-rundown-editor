...
import { DBPieceTypeManifest, IpcOperation, IpcOperationType, PieceTypeManifest } from '../interfaces'
import { db } from '../db'
import { v4 as uuid } from 'uuid'
import { RunResult } from 'sqlite3'

export const mutations = {
	async create (payload: any): Promise<{ result?: PieceTypeManifest, error?: Error }> {
	...
	async read (payload: any): Promise<{ result?: PieceTypeManifest | PieceTypeManifest[], error?: Error }> {
		if (payload && payload.id) {
			const document = await new Promise<DBPieceTypeManifest>((resolve, reject) => db.get(`
				SELECT *
			...
			const documents = await new Promise<DBPieceTypeManifest[]>((resolve, reject) => db.all(`
				SELECT *
				FROM pieceTypeManifests
			`, (e, r) => e ? reject(e) : resolve(r)))
	...
	async update (payload: any): Promise<{ result?: PieceTypeManifest, error?: Error }> {
		const update = {
			...payload.update,
			id: null,
		}
		...
		if (result) {
			const { result: returnResult, error } = await mutations.read({ id: payload.update.id })
