...
ipcMain.handle('playlists', async (_, operation: IpcOperation) => {
	if (operation.type === IpcOperationType.Create) {
		...
		if (result) {
			const document = await new Promise<DBPlaylist>((resolve, reject) => db.get(`
				SELECT *
				FROM playlists
				WHERE id = ?
				LIMIT 1;
		...
		if (operation.payload && operation.payload.id) {
			...
			const documents = await new Promise<DBPlaylist[]>((resolve, reject) => db.all(`
				SELECT *
				FROM playlists
			`, (e, r) => e ? reject(e) : resolve(r)))
